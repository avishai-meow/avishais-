module.exports = function SalchySummonerMaster(script) {	
    ...
    Object.keys(CONTRACTS).forEach(key => script.command.add(key, () => {
        script.toServer('C_REQUEST_CONTRACT', 1, CONTRACTS[key]);
    }));


    script.command.add("3", () => {
        script.toClient('S_NPC_MENU_SELECT', 1, { type:28 });
    });

 script.command.add("contact", (gameId) => {
        try {
        ...
        if (gameId) {
            command.message(`Trying to contact NPC with ID: ${gameId}`);
             script.toServer('C_NPC_CONTACT', 2, { gameId });
        } else {
             script.hookOnce('C_NPC_CONTACT', 2, event => {
                console.log(event);
                command.message(`Contacted NPC with ID: ${event.gameId}`);
            });
        }
    });
